...
{  
  Serial.begin(57600);
    ...
    LEDS.addLeds<WS2812B, LED_OUT, RGB>(ledsRGB, getRGBWsize(LED_COUNT));
  ...
  while(true) {
    ...
    if(Serial.available() > 2) {
      ...
      for (uint8_t x=0; x<3; x++) { // Read three incoming bytes
        ...
        if (x == 3) {   // If we received four serial bytes
          if(pixelIndex == LED_COUNT) break; // Prevent overflow by ignoring the pixel data beyond LED_COUNT
          leds[pixelIndex] = CRGBW(buffer[0], buffer[1], buffer[2], buffer[3]);  //use the FastLED CRGW structure instead of RGB & include another buffer for the color space
          pixelIndex++;
        }
      }
    }
